...
const MISSIONS: Mission[] = [
  { title: 'İlk ışığını yak', concept: 'Kapalı devre', templateId: 'simple', checks: ['Pil ve ampul kullan', 'Kesintisiz bir yol oluştur', 'Ampulden akım geçir'], en: { title: 'Light your first bulb', concept: 'Closed circuit', checks: ['Use a battery and bulb', 'Create an unbroken path', 'Pass current through the bulb'] }, validate: c => c.counts.battery >= 1 && c.counts.bulb >= 1 && hasCurrent(c.currents.bulb) && !c.paused },
  { title: 'Kontrol sende', concept: 'Anahtar', templateId: 'simple', checks: ['Devreye anahtar ekle', 'Anahtarı kapat', 'Ampulü yeniden yak'], en: { title: 'Take control', concept: 'Switch', checks: ['Add a switch', 'Close the switch', 'Light the bulb again'] }, validate: c => c.counts.switch >= 1 && c.switchClosed && hasCurrent(c.currents.bulb) && !c.paused },
...
export const LearningPanel: React.FC = () => {
  ...
  if (!isOpen) return <button className="mission-pill" onClick={() => setIsOpen(true)}><Sparkles size={18} /> {en ? 'Mission' : 'Görev'} {missionIndex + 1}/10</button>;
  ...
  return (
    <aside className="learning-panel" aria-label={en ? 'Learning mission' : 'Öğrenme görevi'}>
      ...
      <div className={`result-card ${completed ? 'success' : ''}`} aria-live="polite">
        {journeyComplete && missionIndex === 9 ? <Trophy size={22} /> : completed ? <CheckCircle2 size={22} /> : <Sparkles size={22} />}<div><strong>{journeyComplete && missionIndex === 9 ? (en ? 'You are a Circuit Master!' : 'Devre Ustası oldun!') : completed ? (en ? 'Mission complete!' : 'Görev tamamlandı!') : (en ? 'Your turn' : 'Sıra sende')}</strong><span>{completed ? (en ? 'Your circuit stays in the workspace. Review the measurements, then continue when ready.' : 'Devre çalışma alanında kalacak. Ölçümleri inceleyip hazır olduğunda sonraki göreve geç.') : (en ? 'Build the circuit or load the starter circuit and experiment.' : 'Devreyi kur veya başlangıç devresini yükleyip üzerinde çalış.')}</span></div>
      </div>
      ...
      <div className="mission-navigation">
        ...
        <span>{completedMissions.length}/10 {en ? 'complete' : 'tamamlandı'}</span>
