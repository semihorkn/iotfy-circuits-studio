...
import { CheckCircle2, ChevronRight, Circle, Flag, RotateCcw, Sparkles, Trophy, X } from 'lucide-react';
...
export const LearningPanel: React.FC = () => {
  ...
  useEffect(() => {
    if (journeyComplete || completed) return;
    const timer = window.setInterval(() => {
      ...
      if (mission.validate(liveContext)) setCompleted(true);
    }, 220);
    return () => window.clearInterval(timer);
  }, [mission, completed, journeyComplete, state.components, state.isPaused, engine]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (missionIndex === MISSIONS.length - 1) {
        ...
        localStorage.setItem('iotfy-mission', String(next));
        clearBoard();
      }
      setCompleted(false);
    }, 1500);
    return () => window.clearTimeout(timer);
  }, [completed, missionIndex, clearBoard]);

  const resetJourney = () => {
    localStorage.removeItem('iotfy-mission'); localStorage.removeItem('iotfy-mission-complete');
    setMissionIndex(0); setJourneyComplete(false); setCompleted(false); clearBoard();
  };
  ...
  if (journeyComplete) return (
    <aside className="learning-panel journey-complete" aria-label="Öğrenme görevi">
      <Trophy size={42} /><h2>Devre Ustası oldun!</h2><p>10 görevin tamamını bitirdin ve bütün parçaları çalıştırdın.</p>
      <button className="example-button" onClick={resetJourney}><RotateCcw size={15} /> Görevleri yeniden başlat</button>
    </aside>
  );
  ...
  return (
    <aside className="learning-panel" aria-label="Öğrenme görevi">
      ...
      <div className="mission-progress"><span style={{ width: `${((missionIndex + (completed ? 1 : 0)) / MISSIONS.length) * 100}%` }} /></div>
      ...
      <div className={`result-card ${completed ? 'success' : ''}`} aria-live="polite">
        {completed ? <CheckCircle2 size={22} /> : <Sparkles size={22} />}<div><strong>{completed ? 'Görev tamamlandı!' : 'Sıra sende'}</strong><span>{completed ? 'Bir sonraki görev hazırlanıyor…' : 'Devreyi kur veya başlangıç devresini yükleyip üzerinde çalış.'}</span></div>
      </div>
